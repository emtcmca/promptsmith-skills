...
/**
 * The upstream paths this mirror actually depends on.
 *
 * The first four are read directly: every generated byte comes from one of them.
 * `evals/` is here because the stamp makes a COUNTED CLAIM about it ("upstream
 * carries N eval cases") and pins links to it, so a change there dates the stamp
 * even though no prompt body moved.
 *
 * Anything upstream that is NOT in this list -- README, CONTRIBUTING, ROADMAP,
 * docs/, .github/ -- cannot change a single byte of output here, so it must not
 * move the stamp. Stamping HEAD instead made every upstream doc commit look like
 * mirror drift. A drift alarm that fires on changes it can prove are irrelevant
 * is one you learn to ignore, and then it is not an alarm.
 */
const SOURCE_PATHS = ['agents', 'skills', 'lenses', 'templates', 'evals']
...
function readSourceCommit() {
  ...
  let sha, shortSha, date, dirty, onRemote, headShort
  try {
    headShort = git(['rev-parse', '--short', 'HEAD'])
    // The last commit that touched anything this mirror reads or cites -- NOT
    // HEAD. See SOURCE_PATHS above for why.
    sha = git(['log', '-1', '--format=%H', 'HEAD', '--', ...SOURCE_PATHS])
    if (!sha) {
      fail(
        `ERROR: no commit in ${SRC} touches any of: ${SOURCE_PATHS.join(', ')}.`,
        `There is nothing here to mirror, so there is no honest commit to stamp.`,
      )
    }
    shortSha = git(['rev-parse', '--short', sha])
    date = git(['log', '-1', '--format=%cs', sha]) // YYYY-MM-DD, committer date
    // Scoped to the same paths: an uncommitted upstream README edit cannot change
    // the output, so it must not raise the "stamp is a lie" warning either.
  ...
  return { sha, shortSha, date, dirty, headShort }
}
 ...
 */
function countEvals(sha) {
  // Read the tree AT the stamped commit rather than off disk. The stamp says
  // "at commit X upstream carries N cases" and pins its links to X, so counting
  // anywhere else -- a later HEAD, or an uncommitted working tree -- produces a
  // number the reader's own click disproves.
  const count = (dir, pattern) => {
    let out
    try {
      out = execFileSync('git', ['ls-tree', '-r', '--name-only', sha, '--', dir], {
        cwd: SRC,
        encoding: 'utf8',
    ...
    if (!out) return 0
    // ls-tree emits one path per line, and paths here contain no whitespace,
    // so splitting on whitespace is both correct and CRLF-proof.
    return out
      .split(/\s+/)
      .filter(Boolean)
...
function stampReadme({ commit, evals }) {
  ...
  const block =
    ...
    `That commit is the most recent one to touch anything this mirror reads or cites; promptsmith's ` +
    `own \`main\` may be further ahead on changes that cannot affect these files.\n` +
    `${END}`
...
function main() {
  ...
  const evals = countEvals(commit.sha)
  ...
  if (commit.dirty) {
    console.warn(
      `WARNING: ${SRC} has uncommitted changes under the mirrored paths. The stamp will ` +
        `say ${commit.shortSha}, but the generated output reflects the working tree, ` +
        `not that commit.`,
    )
  }

  if (commit.shortSha !== commit.headShort) {
    console.log(
      `  upstream HEAD is ${commit.headShort}; stamping ${commit.shortSha}, the last commit ` +
        `touching ${SOURCE_PATHS.join('/, ')}/. Commits in between changed nothing this ` +
        `mirror reads.`,
    )
  }
